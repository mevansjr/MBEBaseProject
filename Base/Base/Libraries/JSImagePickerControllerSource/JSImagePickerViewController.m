...
-(void)updateButtons {
    if (selectedPhotos.count > 0) {
        if (_isMultiple) {
            [self.photoLibraryBtn setTitle:[NSString stringWithFormat:@"Upload %lu photos", (unsigned long)selectedPhotos.count] forState:UIControlStateNormal];
        }
        else {
            [self.photoLibraryBtn setTitle:[NSString stringWithFormat:@"Photo Library"] forState:UIControlStateNormal];
        }
    } else {
        [self.photoLibraryBtn setTitle:@"Photo Library" forState:UIControlStateNormal];
    }
